...
function StatCard({ label, value, sub, icon: Icon, color, delay = 0 }) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ delay }}
      className="glass-card p-6 relative overflow-hidden group hover:border-brand-500/20 transition-all"
    >
      ...
      <div className="font-display font-black text-3xl text-dark-100 mb-1">{value}</div>
      <div className="font-medium text-dark-300 text-sm">{label}</div>
  ...
  return (
    <div>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 py-16 space-y-14">
        {/* Key stats */}
        <div>
          <h2 className="font-display font-bold text-dark-100 text-2xl mb-6">Key Statistics — 2024</h2>
        ...
        {/* Year-wise trend */}
        <div>
          <h2 className="font-display font-bold text-dark-100 text-2xl mb-6">Year-wise Placement Trends</h2>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-dark-700">
                {yearlyData.map((row, idx) => (
                  <tr key={row.year} className={`hover:bg-brand-50/50 transition-colors ${row.year === '2024' ? 'bg-brand-50' : ''}`}>
                    <td className="p-4 text-dark-100 font-bold">{row.year} {row.year === '2024' && <span className="badge-new text-xs ml-2">Latest</span>}</td>
                    <td className="p-4 text-brand-700 font-bold text-right">₹{row.highest} LPA</td>
                    <td className="p-4 text-green-600 font-semibold text-right">₹{row.average} LPA</td>
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <span className="text-dark-100 font-semibold text-sm">{row.placed}%</span>
                        {idx > 0 && (
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div>
            <h2 className="font-display font-bold text-dark-100 text-2xl mb-6">Sector Distribution</h2>
            <div className="glass-card p-6 space-y-4">
              {sectorDistribution.map(({ sector, percentage }) => (
                <div key={sector}>
                  <div className="flex justify-between text-sm mb-1.5">
                    <span className="text-dark-300">{sector}</span>
                    <span className="text-dark-100 font-bold">{percentage}%</span>
                  </div>
                  <div className="h-2 bg-dark-700 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      className="h-full bg-gradient-to-r from-brand-700 to-brand-50 rounded-full"
                    />
                  </div>
                </div>
              ))}
          ...
          <div>
            <h2 className="font-display font-bold text-dark-100 text-2xl mb-6">Student Testimonials</h2>
              ...
              {testimonials.map((t, idx) => (
                <div key={idx} className="glass-card p-5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-dark-100 font-semibold text-sm">{t.name}</div>
                      <div className="text-brand-700 text-xs">{t.company} · {t.package}</div>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {/* Recruiter wall */}
        <div>
          <h2 className="font-display font-bold text-dark-100 text-2xl mb-6">Our Recruiters</h2>
          <div className="glass-card p-6">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {topRecruiters.map((r) => (
                <div
                  key={r.name}
                  className="glass-card p-4 text-center hover:border-brand-500/20 hover:bg-brand-50/30 transition-all cursor-default group"
                >
                  <div className="font-bold text-dark-100 text-sm group-hover:text-brand-700 transition-colors">{r.name}</div>
                  <div className="text-dark-500 text-xs mt-0.5">{r.category}</div>
                  <div className="text-green-600 text-xs font-semibold mt-1">{r.offers} offers</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="glass-card p-8 text-center">
          <h2 className="font-display font-bold text-dark-100 text-2xl mb-3">Training & Placement Cell</h2>
